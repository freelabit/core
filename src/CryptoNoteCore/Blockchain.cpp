...
difficulty_type Blockchain::getDifficultyForNextBlock() {
  ...
  uint8_t blockMajorVersion = get_block_major_version_for_height(static_cast<uint64_t>(m_blocks.size()));
  uint64_t difficultyBlocksCount;
  if(blockMajorVersion < 3){
    difficultyBlocksCount = static_cast<uint64_t>(m_currency.difficultyBlocksCount());
  }
  else {
    difficultyBlocksCount = static_cast<uint64_t>(m_currency.difficultyBlocksCountV3());
  }
  
  //!!!
  size_t offset = m_blocks.size() - std::min(m_blocks.size(), difficultyBlocksCount);
  if (offset == 0) {
    ++offset;
  }
  ...
  if(blockMajorVersion < 3){
    logger(DEBUGGING) << "Using old algo";
    return m_currency.nextDifficulty(timestamps, commulative_difficulties);
  }

  logger(DEBUGGING) << "Using zawy's LWMA difficulty algo";
...
uint8_t Blockchain::get_block_major_version_for_height(uint64_t height) const {
  ...
  } else if (height > m_upgradeDetector.upgradeHeight()) {
    return m_upgradeDetector.targetVersion();
  } else {
...
bool Blockchain::check_block_timestamp_main(const Block& b) {
  ...
  uint8_t blockMajorVersion = get_block_major_version_for_height(static_cast<uint64_t>(m_blocks.size()));
  if(blockMajorVersion == 3)
    ftl = m_currency.blockFutureTimeLimit_V3();
